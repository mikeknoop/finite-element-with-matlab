...
%
% need to get global (x1, y1), (x2, y2) coordinates for this el
x1 = x(lnodes(1), 1);
y1 = x(lnodes(1), 2);
x2 = x(lnodes(2), 1);
y2 = x(lnodes(2), 2);
L = sqrt((x2-x1)^2+(y2-y1)^2); % el length
m = (x2-x1)/L;
n = (y2-y1)/L;
te = [m n 0 0; 0 0 m n];
%
...
case 1
    ...
    therm = ((eprop(1)*eprop(2)*eprop(3)*eprop(4))/L)*([-m -n m n]');
    eldat = therm;
case 2
